...
static double inputDouble(const std::string& prompt, double min = 0.0) {
    double value;
    while (true) {
        std::cout << prompt;
        ...
        std::cout << "Ошибка ввода! Введите число >= " << min << "\n";
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    }
}
...
int main() {
    ...
    while (running) {
        ...
        case 2: {
            ...
            double monthlyFee = inputDouble("Введите абонентскую плату (руб./мес.): ");
